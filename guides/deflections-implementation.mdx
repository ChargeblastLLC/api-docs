...
<Tabs>
  ...
  <Tab title="Pull Data">
    ...
    For request headers, HMAC verification, wire-format rules, required fields, and an Order Insight–style alternative response, see the full guide: **[Digital receipt lookup (merchant HTTPS endpoint)](/guides/digital-receipt-lookup)**.

    ### Implementation Steps

    <Steps>
      <Step title="Implement the HTTPS POST handler">
        Build your endpoint per the [digital receipt lookup guide](/guides/digital-receipt-lookup): verify `X-Digital-Receipt-Lookup-Key` and/or `X-Digital-Receipt-Signature`, read the **raw body** for HMAC, match on fields such as `cardBin`, `cardLast4`, `amount`, `currency`, `transactionDate`, `arn`, etc.
      </Step>
      ...
      <Step title="Return a minimal receipt (200) or not found (404)">
        ...
        You can instead return a full **Order Insight-style** document with a top-level **`receipt`** key; see the [detailed guide](/guides/digital-receipt-lookup#8-full-order-insight-style-response).
      </Step>
    </Steps>
...
- [Digital receipt lookup (HTTPS)](/guides/digital-receipt-lookup) — Full pull-method spec: POST body, headers, HMAC, minimal vs Order Insight responses
