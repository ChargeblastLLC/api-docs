...
The JSON is derived from Chargeblast’s internal **Order Insight–style search object**, with a few rules that matter for **correct HMAC verification**.

### 3.1 Removed keys (never present on the wire)

These keys are stripped before serialization:
...
- The nested object **`destinationAmount`** is **removed**.
- Its **`amount`** and **`currency`** are copied to the **top level** as **`amount`** and **`currency`**.
- **`transactionAmount`** is **removed** from the wire object.

So the merchant-visible body uses top-level **`amount`** / **`currency`** (major units, e.g. `20.5` and `"USD"`), not `transactionAmount` / `destinationAmount`.
...
### 3.4 Typical fields you will see

All are JSON values; many are optional depending on the source transaction. Common fields include (camelCase as encoded):

| Field | Notes |
|--------|--------|
| `source` | Channel / source string when present |
| `cardBin` | First 6 of PAN when present |
| `cardLast4` | Last 4 when present |
| `paymentType` | e.g. card network label |
| `token` | When present |
| `transactionDate` | ISO-8601-style string when present |
| `amount` | **Number**, major currency units (after flattening) |
| `currency` | **String**, e.g. `"USD"` |
| `arn` | Acquirer reference when present |
| `authCode` | When present |
| `terminalId`, `acquirerBin`, `cardAcceptorId`, `mcsn`, `purchaseIdentifier`, `transactionId`, `transactionType`, `cardExpirationDate`, `paymentDescriptor`, `mcc`, `posEntryModeCode`, `eci` | Optional strings / values as applicable |

Exact presence depends on the upstream lookup request.
...
| Status | Meaning | Body |
|--------|---------|------|
| **200**–**299** | Transaction **found** (success). | JSON receipt — either **minimal merchant shape** or full **Order Insight `receipt`** document (see below). |
...
You do **not** need a top-level `response` wrapper. **`responseStatus` in JSON is optional** for minimal shape; found vs not found is primarily driven by **HTTP status** (200 vs 404). Optional `responseStatus` can still be used for cases like multiple matches when you encode that in your contract.
...
- Other CE-related gaps are derived from the mapped Order Insight shape (merchant descriptors, product description, customer identifiers, delivery address, etc.).
...
## 8. Full Order Insight-style response

Instead of the minimal `{ order, merchantProfile, accountProfile }` shape, you may return JSON that decodes as an **Order Insight-style** payload with a top-level **`receipt`** (and related merchant/customer/device objects). Chargeblast branches on the presence of top-level keys (`order` vs `receipt`) when decoding.

Use this when you already emit OI-compatible receipts from another system.

---

## 9. Example minimal body (illustrative)
...
## 11. SLA and deflection eligibility
