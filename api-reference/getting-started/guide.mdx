...
There are multiple ways to programmatically integrate Chargeblast. This guide is intended for resellers, SaaS tools, payment aggregators, and other merchant of merchant businesses. If you are a merchant yourself, please see our [webhooks](/api-reference/webhooks/setup), which should be sufficient for most use cases.
...
- Implement [webhook](/api-reference/webhooks/setup) to receive alert
- Perform alert to transaction matching internally, refunding cardholder if desired and then update the alert via the [/update/alert](/api-reference/alerts/update-alert) endpoint
