...
**Deflection events:** payloads use the deflection body schema (not the alert schema). See [Webhooks](/api-reference/webhooks/setup#body) (Deflection tab).
</Info>
...
=======
| `deflection.updated` | CE, FPT | An existing deflection changes in a way that should notify subscribers. | Includes CE status transitions (for example Succeeded → Reversed) and billing updates when `fees` is set. |
>>>>>>> Stashed changes
