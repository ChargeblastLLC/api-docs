...
| `deflection.updated` | CE, FPT | An existing deflection changes in a way that should notify subscribers. | **CE** status transitions only (for example Succeeded → Reversed). Billing (`fees` / `pushedToBilling`) does **not** emit. |
...
Fires when an existing deflection is updated for subscribers — a **CE** status change from a later Verifi notification (for example Succeeded → Reversed / Failed / Timeout). Billing updates do **not** emit this event. Same deflection body schema as `deflection.created`.
