...
| `alert.will_expire` | Ethoca, CDRN | Fires approximately **46–48 hours before** the alert expires, for eligible **WIP** alerts that still need attention (for example match data present but no matched charge yet). | **RDR** and **TC40** are not selected for this event. **At most once per alert** (deduped). |
...
**Ethoca and CDRN only.** A scheduled job emits this approximately **46–48 hours before** the alert expires, for eligible **WIP** alerts that still need attention (for example match-related data exists but there is still **no** matched charge id). **RDR** and **TC40** are **not** included. Each alert receives this event **at most once** (deduped).
