...
Fires when Chargeblast first persists a deflection (**CE** or **FPT**). The JSON body is the [deflection schema](/api-reference/webhooks/setup#body) (Deflection tab), not an alert object. **`type`** is `CE` or `FPT`; **`status`** is one of `Succeeded`, `Reversed`, `Failed`, or `Timeout`.
