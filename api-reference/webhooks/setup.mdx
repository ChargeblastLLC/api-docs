...
Chargeblast emits several **`X-Event-Type`** values (for example `alert.created`, `alert.updated`, `alert.refunded`, `alert.matched`, `alert.will_expire`, `alert.within_refund_rules`). Which categories apply, when each event fires, and deduping behavior are documented here:
...
- [Webhook event types](/api-reference/webhooks/webhook-events) — Full event-type reference (Ethoca, CDRN, RDR, TC40)
