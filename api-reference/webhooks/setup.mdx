...
Alert events (`alert.*`) use the alert schema below. Deflection events (`deflection.*`) use the [deflection body](#deflection-body) instead.
...
Chargeblast will begin emitting events to your webhook URL. Alert events use the alert schema below; deflection events use a separate deflection schema (see [Deflection body](#deflection-body)).
...
When **`X-Event-Type`** is `deflection.created` or `deflection.updated`, the JSON body is a **deflection** object (not an alert). Fields:
...
<ResponseField name="amountUsd" type="float" placeholder="49.99"></ResponseField>
<ResponseField name="currency" type="string" placeholder="USD"></ResponseField>
<ResponseField
  name="chargeId"
  type="string"
...
<ResponseField
  name="retailerId"
  type="string"
  placeholder="ret_4xUCdBXVs23u03i51cpQBJ"
></ResponseField>
