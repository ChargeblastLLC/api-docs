...
For the full list of webhook event types (`alert.created`, `alert.updated`, `alert.refunded`, and others), when they fire, and which alert categories apply, see **[Webhook event types](/api-reference/webhooks/webhook-events)**. Prefer the **[Webhooks](/api-reference/webhooks/setup)** guide for the current payload schema and verification steps.
