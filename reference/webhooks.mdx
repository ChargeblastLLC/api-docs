...
![](/images/reference/chargeblast-webhook-url-dashboard.png)
...
For the full list of webhook event types (`alert.*` and `deflection.*`), when they fire, and which types apply, see **[Webhook event types](/api-reference/webhooks/webhook-events)**. Prefer the **[Webhooks](/api-reference/webhooks/setup)** guide for the current payload schema and verification steps.
