...
If you are using only payment processors that are supported via our [integration directory](https://www.chargeblast.com/integrations), you can integrate directly via our integrations page and use our **workflow builder** to set up **refund rules**—including branching for **Ethoca / CDRN (issuer) alerts** and, where supported (for example Stripe), **EFW** (**early fraud warning**) rules for auto-refunding or filtering low-ASP transactions. See [Stripe integration](/reference/stripe-integration) for more on EFW rules.
